...
class ArduinoDevice extends events.EventEmitter {
	...
	update ( opts ) {
		...
		if (( this._opts.address != opts.address ) || ( this._opts.port != opts.port )) {
			...
			this.emit('network', {"device":this, "address":this._opts.address, "port":this_opts.port});
		}
	}
	...
	query( name, type, parameter ) {
		let endpoint = type+"/"+name;
		return this.executeRequest(endpoint, parameter)
			.then( (body) => {
				if (( ('error') in body ) && (body.error!="")) return Promise.reject( body.error );
