...
class ArduinoDiscovery extends events.EventEmitter {
		...
		while (i--) {
			...
			if (lastSeenOn < deleteIfLastSeenBefore) {
				if (!this._devices[keys[i]].getOpt('manuallyAdded')) {
					...
					this._debug('Device',keys[i],'could not be discovered but is flagged as manually added.');
				}
			}
		}
	}
	...
	_onServerMessage( message, host ) {		
		...
		if ( !('id' in opts) ) { this._debug("Fatal error: no ID in json data"); return; }
		if ( !('type' in opts) ) { this._debug("Fatal error: no type in json data"); return; }
		if ( !('api' in opts) ) { this._debug("Fatal error: no api in json data"); return; }
